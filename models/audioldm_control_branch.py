...
class AudioLDMControlBranch(nn.Module):
    ...
    def __init__(
        ...
        self.down_zero_convs = nn.ModuleList(
            [conv_builder(nn.Conv2d(hidden_channels, out_ch, kernel_size=1)) for out_ch in input_block_channels]
        )
        self.mid_zero_conv = conv_builder(nn.Conv2d(hidden_channels, middle_block_channel, kernel_size=1))
    ...
    def forward(
        ...
        if hint.shape[-2:] != zt.shape[-2:]:
            hint = F.interpolate(hint, size=zt.shape[-2:], mode="bilinear", align_corners=False)
        ...
        for ds, proj in zip(self.input_block_ds, self.down_zero_convs):
            feat = F.interpolate(base, size=self._target_hw(input_hw, ds), mode="bilinear", align_corners=False)
