...
def _apply_robust_scaler(arr: np.ndarray, *, time_axis: int) -> np.ndarray:
    median = np.median(arr, axis=time_axis, keepdims=True)
    q75 = np.percentile(arr, 75.0, axis=time_axis, keepdims=True)
    q25 = np.percentile(arr, 25.0, axis=time_axis, keepdims=True)
    iqr = q75 - q25
...
def apply_source_level_eeg_preprocessing(
    ...
    if bool(preprocessing.get("robust_scaler", False)):
        out = _apply_robust_scaler(out, time_axis=1).astype(np.float32, copy=False)
...
def apply_chunk_level_eeg_preprocessing(
    ...
    *,
    ...
    if bool(cfg.get("robust_scaler", False)):
        eeg = _apply_robust_scaler(eeg.astype(np.float32, copy=False), time_axis=1)
